...
## Repo constraints (when used in this repo)
...
- Routing changes must be done in `src/app/AppRouter.tsx`.
- API calls must go through `src/api/http.ts` wrappers (no direct `fetch` in components).
- Use Tailwind utility classes and existing UI patterns (don’t introduce a new design system).

Cross-layer coordination:
- If the request depends on backend API changes/contracts, route through `.github/copilot/agents/orchestrator.md`.
